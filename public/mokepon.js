...
const spanMascotaImgJugador = document.getElementById('img-jugador')
const spanMascotaImgEnemigo = document.getElementById('img-enemigo')
...
alturaQueBuscamos = anchoDelMapa
...
{
    fetch("http://192.168.10.174:8080/unirse")
        .then(function (res) {
            console.log(res)
            if(res.ok)
            {
...
function seleccionarMascota()
{
    if(inputGorila.checked)
    {
        ...
        spanMascotaImgJugador.src = "./assets/gorila.png"
        spanMascotaImgJugador.style.width = "80px"
        spanMascotaImgJugador.style.height = "100px"
    }
    else if(inputLeopardo.checked)
    {
        spanMascotaJugador.innerHTML = inputLeopardo.id
        mascotaJugador = inputLeopardo.id
        spanMascotaImgJugador.src = "./assets/leopardo.png"
        spanMascotaImgJugador.style.width = "80px"
        spanMascotaImgJugador.style.height = "100px"
    }
    else if(inputZorro.checked)
    {
        ...
        spanMascotaImgJugador.src = "./assets/zorro.png"
        spanMascotaImgJugador.style.width = "80px"
        spanMascotaImgJugador.style.height = "100px"
    }
    else
...
{
    fetch("http://192.168.10.174:8080/mokepon/" + jugadorId, {
        method: "post",
        headers: {
...
function seleccionarMascotaEnemigo(enemigo)
{
    ...
    spanMascotaImgEnemigo.src = `./assets/${enemigo.nombre}.png` 
    spanMascotaImgEnemigo.style.width = "80px"
    spanMascotaImgEnemigo.style.height = "100px"
...
{   
    ...
    // ataqueAirebt = document.getElementById('boton-aire')
    // ataqueLavabt = document.getElementById('boton-lava')
...
{
    fetch(`http://192.168.10.174:8080/mokepon/${jugadorId}/ataque`, {
        method: "post",
        headers:{
            "Content-Type": "application/json"
        },
...
{
    fetch(`http://192.168.10.174:8080/mokepon/${enemigoId}/ataque`)
        .then(function(res){
            if(res.ok)
            {
                res.json()
...
function botonesFinJuego()
{
    ataqueFuegobt.disabled = true
    ataqueFuegobt.style.backgroundColor = "#A9A9A9"
    ataqueAguabt.disabled = true
    ataqueAguabt.style.backgroundColor = "#A9A9A9"
    ataqueTierrabt.disabled = true
    ataqueTierrabt.style.backgroundColor = "#A9A9A9"
}
...
{
    fetch(`http://192.168.10.174:8080/mokepon/${jugadorId}/posicion`, {
        method: "post",
        headers: {
            "Content-Type": "application/json"
        },
